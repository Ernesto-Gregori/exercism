...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    criticality_neutrons = 500
    criticality_product = 500000
    product = temperature * neutrons_emitted
    
    if temperature < criticality_temperature and neutrons_emitted > criticality_neutrons and product < criticality_product:
        return True
    return False
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if product < threshold_low:
        return 'LOW'
    # if product >= threshold_low and product <= threshold_high:
    if threshold_low <= product <= threshold_high:
        return 'NORMAL'
    return 'DANGER'
